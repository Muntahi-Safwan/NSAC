...
def start_scheduler():
    """Start the NSAC data collection scheduler service"""
    print("🚀 Starting NSAC Data Collection Scheduler...")
    success, stdout, stderr = run_command(
        "docker-compose up -d nsac-scheduler"
    )
    
    if success:
        print("✅ NSAC Scheduler started successfully")
        print("📊 Service will run Air Quality and Wildfire collections every hour")
    else:
...
def stop_scheduler():
    """Stop the NSAC data collection scheduler service"""
    print("🛑 Stopping NSAC Scheduler...")
    success, stdout, stderr = run_command(
        "docker-compose stop nsac-scheduler"
    )
    
    if success:
        print("✅ NSAC Scheduler stopped")
    else:
...
def show_status():
    ...
    print("📊 NSAC Services Status:")
    print("=" * 40)
    
    # Check scheduler
    success, stdout, stderr = run_command(
        "docker-compose ps nsac-scheduler", check=False
    )
    if "Up" in stdout:
        print("🟢 NSAC Scheduler: Running (Air Quality + Wildfire)")
    else:
        print("🔴 NSAC Scheduler: Stopped")
...
def show_logs():
    ...
    print("📋 Recent NSAC Scheduler Logs:")
    print("=" * 40)
    
    success, stdout, stderr = run_command(
        "docker-compose logs --tail=20 nsac-scheduler"
    )
...
def main():
    """Main entry point"""
    if len(sys.argv) < 2:
        print("NSAC Services Management")
        print("=" * 40)
        print("Usage: python manage_services.py <command>")
        print()
        print("Commands:")
        print("  start-scheduler    Start the hourly data collection scheduler (Air Quality + Wildfire)")
